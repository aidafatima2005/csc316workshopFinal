...
public class AdjacencyListGraph<V, E> extends EdgeListGraph<V, E> {
    ...
    /**
     * Creates a new edge with the given data and vertices.
     *
     * @param data the data to store in the edge
     * @param vertex1 the first vertex of the edge
     * @param vertex2 the second vertex of the edge
     * @return a new {@code ALEdge} connecting {@code vertex1} and {@code vertex2} with the given data
     */
    ...
    public Edge<E> insertEdge(Vertex<V> vertex1, Vertex<V> vertex2, E edgeData) {
        ...
        ALEdge edge = (ALEdge) super.insertEdge(v1, v2, edgeData);
        
        Position<Edge<E>> outPos = v1.getOutgoing().addLast(edge);
        edge.setOutgoingListPosition(outPos);
        
        Position<Edge<E>> inPos = v2.getIncoming().addLast(edge);
        edge.setIncomingListPosition(inPos);
    ...
    public void removeEdge(Edge<E> edge) {
        ...
        // Remove the edge from v1's outgoing list using its stored position.
        if (e.getOutgoingListPosition() != null) {
            v1.getOutgoing().remove(e.getOutgoingListPosition());
        }
        
        // Remove the edge from v2's incoming list using its stored position.
        if (e.getIncomingListPosition() != null) {
