...
public class GraphTraversalUtil {
    ...
    public static <V, E> Map<Vertex<V>, Edge<E>> depthFirstSearch(Graph<V, E> graph, Vertex<V> start) {
    	Set<Vertex<V>> s = new HashSet<Graph.Vertex<V>>();
    	Map<Vertex<V>, Edge<E>> m = new LinearProbingHashMap<Graph.Vertex<V>, Graph.Edge<E>>();
    	dfsHelper(graph, start, s, m);
    	return m;
    }
    
    private static <V, E> void dfsHelper(Graph<V, E> graph, Vertex<V> u, 
    		Set<Vertex<V>> known, Map<Vertex<V>, Edge<E>> forest) {
    	known.add(u);
    	for (Edge<E> e : graph.outgoingEdges(u)) {
    		Vertex<V> uNew = u;
    		uNew = graph.opposite(uNew, e);
    		if (!known.contains(uNew)) {
    			forest.put(uNew, e);
    			dfsHelper(graph, uNew, known, forest);
    		}
    	}
    }
    ...
    public static <V, E> Map<Vertex<V>, Edge<E>> breadthFirstSearch(Graph<V, E> graph, Vertex<V> start) {
    	...
    	Queue<Vertex<V>> q = new ArrayBasedQueue<Graph.Vertex<V>>();
