...
public class HeapAdaptablePriorityQueue<K extends Comparable<K>, V> extends HeapPriorityQueue<K, V>
    ...
    public void remove(Entry<K, V> entry) {
    	AdaptablePQEntry<K, V> entryToRemove = validate(entry);
    	int j = entryToRemove.getIndex();
    	if (j == list.size() - 1) {
    ...
    public void replaceKey(Entry<K, V> entry, K key) {
    	AdaptablePQEntry<K, V> entryToReplace = validate(entry);
    	entryToReplace.setKey(key);
    	bubble(entryToReplace.getIndex());
    }

    @Override
    public void replaceValue(Entry<K, V> entry, V value) {
    	AdaptablePQEntry<K, V> entryToReplace = validate(entry);
    	entryToReplace.setValue(value);
    }
}
