...
public class HeapPriorityQueue<K extends Comparable<K>, V> extends AbstractPriorityQueue<K, V> {
    ...
    /** 
     * Adds the entry with key K and value V 
     * 
    ...
    public Entry<K, V> min() {
        ...
        for (int i = 0; i < list.size(); i++) {
        	K currentKey = list.get(i).getKey();
        	if (compare(currentKey, min.getKey()) < 0) {
        		min = list.get(i);
        	}
        }
        return min;
    ...
    protected void upHeap(int index) {
    	...
    	while (idx > 0) {
    		int p = parent(idx);
    		if (compare(list.get(idx).getKey(), list.get(p).getKey()) >= 0) {
    			break;
    		}
    		swap(idx, p);
    		idx = p;
    ...
    protected void downHeap(int index) {
    	...
    	while (hasLeft(idx)) {
    		...
    		if (compare(list.get(smallChildIndex).getKey(), list.get(idx).getKey()) >= 0) {
    			break;
    		}
    		swap(idx, smallChildIndex);
    		idx = smallChildIndex;
    	}
    }
}
