...
public class HeapAdaptablePriorityQueueTest {
	...
	/** HeapAdaptablePriorityQueue variable for test */
    ...
    public void testStudentHeap() {
        ...
        Entry<Student, String> entry1 = sHeap.insert(s1, "s1");
        Entry<Student, String> entry2 = sHeap.insert(s2, "s2");
        Entry<Student, String> entry3 = sHeap.insert(s3, "s3");
        Entry<Student, String> entry4 = sHeap.insert(s4, "s4");
        Entry<Student, String> entry5 = sHeap.insert(s5, "s5");
        assertEquals(5, sHeap.size());
        
        // Initially, the minimum should be s1 (id=1)
        assertEquals(s1, sHeap.min().getKey());
        assertEquals("s1", sHeap.min().getValue());
        
        // Replace the key of entry3 with a new Student (id=0)
        Student newStudent = new Student("S", "H", 0, 1, 0, "sh0");
        sHeap.replaceKey(entry3, newStudent);
        
        // Now, the minimum should be the new Student (id=0)
        assertEquals(newStudent, sHeap.min().getKey());
        assertEquals("s3", sHeap.min().getValue());
        
        // Replace the value of entry1
        sHeap.replaceValue(entry1, "S1");
        assertEquals("S1", entry1.getValue());
        
        // Remove entry2 and check the new minimum
        sHeap.remove(entry2);
        ...
        sHeap.replaceValue(entry5, "S5");
        assertEquals("S5", entry5.getValue());
    }
}
