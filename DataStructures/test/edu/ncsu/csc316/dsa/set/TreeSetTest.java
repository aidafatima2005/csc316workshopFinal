...
public class TreeSetTest {
    ...
    public void testRemove() {
        ...
        assertEquals(Integer.valueOf(15), set.remove(15));
        assertEquals(4, set.size());
        assertFalse(set.contains(15));

        assertNull(set.remove(15)); // Already removed
        assertEquals(4, set.size());
    }
    ...
    public void testIterator() {
        ...
        int[] expected = {5, 10, 15, 20, 25}; // assuming in-order traversal
        int index = 0;
        while (it.hasNext()) {
            assertEquals(Integer.valueOf(expected[index++]), it.next());
        }
